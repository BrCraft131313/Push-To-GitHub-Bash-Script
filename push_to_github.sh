...
# 3. معالجة وتحديد اسم المستودع والمالك Target User
if [[ "$REPO_INPUT" =~ ^https://github\.com/([^/]+)/([^/.]+)(\.git)?$ ]]; then
    # إذا تم إدخال رابط كامل
    TARGET_OWNER="${BASH_REMATCH[1]}"
    REPO_NAME="${BASH_REMATCH[2]}"
else
    # إذا تم إدخال اسم المستودع فقط
...
FULL_REPO_URL="https://github.com/$AUTHENTICATED_USER/$REPO_NAME.git"

# 5. التحقق من وجود المستودع على GitHub أو إنشائه
if ! gh repo view "$AUTHENTICATED_USER/$REPO_NAME" &>/dev/null; then
    echo "Repository '$AUTHENTICATED_USER/$REPO_NAME' does not exist. Creating it now..."
    # إنشاء المستودع كعالمي (Public) تلقائياً
    gh repo create "$AUTHENTICATED_USER/$REPO_NAME" --public --confirm || {
        echo "Error: Failed to create repository on GitHub."
        exit 1
    }
fi

# 6. الانتقال إلى المجلد وتنفيذ الرفع
cd "$FOLDER_PATH" || exit
